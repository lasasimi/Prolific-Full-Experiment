...
class ExitPage(Page):
    ...
    def js_vars(player: Player):
        ...
        elif not player.participant.treatment_available:
            player.participant.reason="Sorry, we could not proceed due to mismatch of your answer to the pre-survey question and your demographic information on Prolific."
        # From mock app
    ...
    def is_displayed(player: Player):
        ...
        if not player.participant.complete_presurvey:
            return not player.participant.active or player.participant.failed_attention_check or not player.participant.training_success or not player.participant.gives_consent or not player.participant.treatment_available
